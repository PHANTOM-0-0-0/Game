...
lt_all = set('abcdefghijklmnopqrstyuvw')
...
while True:
    ...
    if lt == '/':
        if lt == '/help':
            ...
            print('Проверенные буквы: ', str(lt_old))
            continue
        elif lt == '/left':
            print('Непроверенные буквы: ', str(lt_all - lt_old) )
            continue
        else:
            print('Ошибка! Введенная команда неизвестна!')
            continue

    # Теперь точно знаем, что введена не команда
    ntr = ntr + 1

    # проверяем, что ввели слово целиком
    if lt.lower() == word:
        # Угадали слово целиком.
        break
    ...
    # Если буква вводилась, переходим в начало уикла, если нет - то запоминаем букву и продолжаем.
    if lt in lt_old:
